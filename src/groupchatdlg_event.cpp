...
bool GroupChatDlg::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if(obj == ui->btn_close){                          //最小化、最大化、关闭按钮换图
            if(event->type() == QEvent::Enter){
                ...
                if(me->x() > 0 && me->x() < lb->width() && me->y() > 0 && me->y() < lb->height()){
                    this->close();
                    //emit closeGroupChatDlg(roomJid);
                }else{
                    ui->btn_close->setPixmap(QPixmap(":/pixmap/image/closeButton.png"));
                }
            } else {
    ...
    if(obj==ui->closeButton||obj==ui->sendButton)
    {
...
void GroupChatDlg::mousePressEvent(QMouseEvent *event)
{
...
void GroupChatDlg::mouseMoveEvent(QMouseEvent *event)
{
...
void GroupChatDlg::mouseReleaseEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        setWindowOpacity(1);
